 ...
 *
 * Everything the URL needs is in the citation: "R.I. Gen. Laws § 11-23-1" is
 * title 11, chapter 11-23, section 11-23-1.
 *
 *   /Statutes/TITLE11/11-23/11-23-1.htm
 *
 * Honest upstream: an unknown section returns 404 with an empty body.
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Rhode Island General Laws');
}

const BASE = 'https://webserver.rilegislature.gov/Statutes';
const UA = 'pipeworx-mcp-rhode-island-code/1.0 (+https://pipeworx.io)';
...
const tools: McpToolExport['tools'] = [{
  ...
  inputSchema: {
    type: 'object',
    properties: {
      section: { type: 'string', description: 'Section as cited, e.g. "11-23-1" (murder) or "31-27-2" (DUI). Title-chapter-section.' },
    },
    required: ['section'],
  },
}];
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  if (name !== 'ri_general_law') throw new Error(`Unknown tool: ${name}`);
  const raw = String(args.section ?? '').trim().replace(/\s+/g, '');
  const m = /^(\d+[A-Za-z]?)-(\d+[A-Za-z.]*)-(\d+[A-Za-z.\-]*)$/.exec(raw);
  if (!m) {
    return { found: false, reason: 'unparsed_section',
      message: `"${args.section}" is not a Rhode Island citation.`,
      hint: 'RI sections are title-chapter-section, e.g. "11-23-1" (murder) or "31-27-2" (DUI).' };
  }
  const title = m[1];
  const url = `${BASE}/TITLE${title}/${title}-${m[2]}/${raw}.htm`;
  const res = await pwFetch(url, { headers: { 'User-Agent': UA } });
  if (res.status === 404) {
    return { found: false, reason: 'section_not_found', section: raw, title,
      message: `No section ${raw} in the Rhode Island General Laws (HTTP 404).`,
      hint: 'Section numbers are not contiguous — repealed sections are normal.' };
  }
  if (!res.ok) throw new Error(`Rhode Island Legislature returned HTTP ${res.status} for ${raw}`);
  ...
  return {
    citation: `R.I. Gen. Laws § ${raw}`, section: raw, title,
    text, text_chars: text.length, url,
    source: 'Rhode Island General Assembly (rilegislature.gov), keyless',
    attribution: 'Rhode Island statutes are public record.',
  };
}
